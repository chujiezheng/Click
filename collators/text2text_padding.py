...
def collate_fn(data_list, toker: PreTrainedTokenizer, max_input_length=None, max_decoder_input_length=None, infer=False):
    ...
    aux_input_ids = torch.tensor([e for f in features for e in f.aux_input_ids], dtype=torch.long)
    aux_attention_mask = torch.tensor([e for f in features for e in f.aux_attention_mask], dtype=torch.float)
    if not infer:
...
def convert_data_to_feature(data, toker: PreTrainedTokenizer, max_input_length, max_decoder_input_length=None) -> Feature:
    ...
    aux_sources = [process(e) for e in data['aux_sources']]
    ...
    aux_input_ids = [e[:MAX_AUX_LENGTH] for e in aux_sources] + [[] for _ in range(MAX_SRC_NUM - len(aux_sources))]
    ...
    aux_attention_mask = [[1.] * len(e) + [0.] * (MAX_AUX_LENGTH - len(e)) for e in aux_input_ids]
    aux_input_ids = [e + [pad_token_id] * (MAX_AUX_LENGTH - len(e)) for e in aux_input_ids]

...
def make_sample(toker, context, generation):
    d = {'source': context, 'target': generation}
    return d
...
def run(args):
    ...
    contexts = [e['source'] for e in contexts]
    for infer_data_path in args.infer_data_paths:
        ...
        if os.path.exists(f'{infer_data_path}/loss_{args.save_name}_list.txt'):
            ...
            continue
        generations =[json.loads(e)['generation'] for e in open(f'{infer_data_path}/gen.txt')]
...
def main():
    ...
    parser.add_argument('--collator_name', type=str, default='gpt2_eval')
